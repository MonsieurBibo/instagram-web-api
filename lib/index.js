...
class Instagram {
  ...
  async getThreads({ limit = 10, messageLimit = 10 }) {
    this.iDotInstagramRequest = this.request.defaults({
      baseUrl: 'https://i.instagram.com',
      headers: {
        'User-Agent':
          "'Mozilla/5.0 (Linux; Android 8.1.0; motorola one Build/OPKS28.63-18-3; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/70.0.3538.80 Mobile Safari/537.36 Instagram 72.0.0.21.98 Android (27/8.1.0; 320dpi; 720x1362; motorola; motorola one; deen_sprout; qcom; pt_BR; 132081645)'" // It uses a compatible Android's IG app User-Agent.
      }
    })
    return this.iDotInstagramRequest
      .get(
        `/api/v1/direct_v2/inbox/?persistentBadging=true&folder=&limit=${limit}&thread_message_limit=${messageLimit}`
      )
      .then(data => data)
      .catch(err => err.error)
  }
